...
function [Uc,C,Ugo,Go,IGo_DA_Ach,Unogo,NoGo,INoGo_DA_Ach,Ugpe,Gpe,Ugpi,Gpi,Ut,T,Ustn,STN,E,t,Wgc_post,Wgs_post,Wnc_post,Wns_post,r,k_reward,ChI,sw] = BG_model_function_Ach_new(S,Wgc,Wgs,Wnc,Wns,Correct_winner,Small_winner,Low_winner_1,Low_winner_2,Dop_tonic,noiseC,gain_drop_dop)
                ...
                if lost==0   %there is al least one winner
                    ...
                    if n_C_winner==1   %there is just one winner, I give reward or punishment
                        ...
                        elseif C_winner_list == Small_winner
                            r=1;%small reward da modificare
                            sw = 2;
                            gain = 0.02;
                        elseif C_winner_list == Low_winner_1
                            r=1;%small reward da modificare
                            sw = 3;
                            gain = 0.02;
                        elseif C_winner_list == Low_winner_2
                            r=1;%small reward da modificare
                            sw = 4;
                            gain = 0.02;
                        else
...
%%
        for l=1:Nc
            delta_Wgc(l,l) = gain*(C(l,k_reward)-C_th(l)).*(val_Go(l)-Go_th(l));
            if (C(l,k_reward)<C_th(l)) && (val_Go(l)<Go_th(l))
                delta_Wgc(l,l) = 0;
            end
            delta_Wnc(l,l) = gain*(C(l,k_reward)-C_th(l)).*(val_NoGo(l)-NoGo_th(l));
            if (C(l,k_reward)<C_th(l)) && (val_NoGo(l)<NoGo_th(l))
                delta_Wnc(l,l) = 0;
            end
        end
        ...
        for row=1:Nc
            for col=1:Nc
                delta_Wgs(row,col) = gain*(val_Go(row)-Go_th(row))*(S(col)-S_th(col));
                if  (val_Go(row)<Go_th(row)) && (S(col)<S_th(col))
                    delta_Wgs(row,col) = 0;
                end
                delta_Wns(row,col) = gain*(val_NoGo(row)-NoGo_th(row))*(S(col)-S_th(col));
                if (val_NoGo(row)<NoGo_th(row)) && (S(col)<S_th(col))
                 delta_Wns(row,col) = 0;   
                end
            end
        end
