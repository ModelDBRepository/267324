...
%
%Description: Script for training synapses: Basal Learning.
%
% 10 independent learning procedures (each with 300 epochs starting from
% the naïve network and different noise realizations, with different seeds
% of noise) are performed.

% Each epoch consists of the four stimuli associated with three reward
% probabilities: P_high = 0.8, P_small = 0.3, P_low = 0.1.
%
...
for ii = 1:N_training
    ...
    Wgc_epocs = zeros(Nc,Nc,Ns*N_epoche);
    Wgs_epocs= zeros(Nc,Nc,Ns*N_epoche);
    Wnc_epocs = zeros(Nc,Nc,Ns*N_epoche);
    Wns_epocs = zeros(Nc,Nc,Ns*N_epoche);
    ...
    vett_reward = zeros(Ns*N_epoche,1);
    vett_punishment = zeros(Ns*N_epoche,1);
    vett_small_winn = zeros(Ns*N_epoche,1);
    vett_low_winn_1 = zeros(Ns*N_epoche,1);
    vett_low_winn_2 = zeros(Ns*N_epoche,1);
    vett_no_risposta = zeros(Ns*N_epoche,1);

    S_vett = zeros(Nc,Ns*N_epoche);
    S1_vett = zeros(Nc,Ns*N_epoche);
    S2_vett = zeros(Nc,Ns*N_epoche);
    S3_vett= zeros(Nc,Ns*N_epoche);
    S4_vett= zeros(Nc,Ns*N_epoche);
    ...
    for i = 1:N_epoche
        ...
        for j = 1:Ns
            ...
            S_vett(3,j+Ns*(i-1)) = S(3);
            S_vett(4,j+Ns*(i-1)) = S(4);
    ...
    save (name, 'Wgc_epocs', 'Wgs_epocs', 'Wnc_epocs', 'Wns_epocs', 'vett_reward', 'vett_punishment', 'vett_no_risposta', 'vett_small_winn','vett_low_winn_1','vett_low_winn_2', 'S_vett', 'Dop_tonic', 'N_epoche','choices')
