...
function [Uc,C,Ugo,Go,IGo_DA_Ach,Unogo,NoGo,INoGo_DA_Ach,Ugpe,Gpe,Ugpi,Gpi,Ut,T,Ustn,STN,E,t,Wgc_post,Wgs_post,Wnc_post,Wns_post,r,k_reward,ChI,sw] = BG_model_function_Ach(S,Wgc,Wgs,Wnc,Wns,Correct_winner,Small_winner,Dop_tonic,noiseC,gain_drop_dop)
...
% It is different from the other functions since it use the positive part for all the pre-synaptic activities 
...
gain_ADHD=1.0;%modificare per ADHD=[1.5,2.0]
...
%%
        for l=1:Nc
            delta_Wgc(l,l) = gain*(C(l,k_reward)*val_Go(l)-Wgc(l,l)*val_Go(l));
            delta_Wnc(l,l) = gain*(C(l,k_reward)*val_NoGo(l)-Wnc(l,l)*val_NoGo(l)); 
        end
        
        for row=1:Nc
            for col=1:Nc
                delta_Wgs(row,col) = gain*(S(col)*val_Go(row)-val_Go(row)*Wgs(row,col));
                delta_Wns(row,col) = gain*(S(col)*val_NoGo(row)-val_NoGo(row)*Wns(row,col));  
            end
        end
    end
